...
% load RF data
load('stDataInfo.mat','stDataInfo');
fid = fopen('mRcvData.bin','rb');
mRcvData = reshape(fread(fid,'single'),stDataInfo.aSize);
fclose(fid);

% transducer info
stTr = stDataInfo.stTrans;
% rf data info
stRf = stDataInfo.stRf;
...
mCavMap_zx    = single(gpuArray(zeros([stG.nZdim,stG.nXdim])));
...
for repidx = 1:20 % for multiple measurements
    tic;
    % call the kernel
...
disp(['Averaged processing time = ' num2str(mean(aTime)) 's']);
